...
function _makeDayAxis(id, vStart, num){
    ...
    g.append(SVG.makeText(null, 20, vStart-50, "Day "+num, 12, 45));
    ...
    for(var i = 0; i <= 24; i=i+4){
        ...
        innerGroup.append(SVG.makeText(tickId + "-label", tickX-15, vStart+20, ""+i+":00", 10, 45));
...
function _makeCallOutBox(events, x, y){
    ...
    g.append(
        SVG.makeLine(null, x, y, x,
            y-((medEvents.length) * EVENT_V_DIST+20),
            "black", 1)
    );
    
    for(var i = 0; i < medEvents.length; i++){
        
        var tmpY = y-((i)*EVENT_V_DIST + 20);
        ...
        g.append(
            SVG.makeLine(null,
                x, tmpY, 
                x - RECT_BACKBONE_SEP, tmpY-RECT_BACKBONE_SEP/2,
                "black", 1
            )
        );
        ...
        g.append(
            SVG.makeText(null,
                x - RECT_BACKBONE_SEP - RECT_W + (RECT_W / 20), tmpY - RECT_H + (RECT_H / 2 + 3),
                events[i].getText(), 10 
            )
        );
    ...
    g.append(
        SVG.makeRoundedRect(null,
            x-RECT_W/4, y-((medEvents.length) * EVENT_V_DIST+20+RECT_H),
            RECT_W/2, RECT_H,
    ...
    g.append(
        SVG.makeText(null,
            x-15, y-medEvents.length*EVENT_V_DIST-RECT_H-7,
            _makeDateString(events[0].time),10
        )
    );
...
function _makeEventMark(hOffset, vOffset, events) {
    ...
    var x = _scaleDayToAxis(events[0].time, hOffset, 880);
