...
//var COLORS = { moon : "#a9c9e8", sun : "#fffcc0", food : "#c2e8ba", medication : "#dcceff", other : '#e8e8e8'};
...
function _makeCallOutBox(events, x, y){
    ...
    for(var i = 0; i < events.length; i++){
        ...
        }else if(events[i] instanceof Event.FoodEvent){
            color = COLORS.food; 
...
function _makeEventMark(hOffset, vOffset, events) {
    ...
    function noHover(){
        ...
        g.mousedown(function(){
            ...
            console.log('mousedown noHover');
            g.children("circle").attr("r", 5);
            g.children("g").remove();
            g.mousedown(enableHover);
        });
    }
    
    function enableHover(){

        g.mouseleave(mouseLeaveHover);
        g.mouseenter(mouseEnterHover);
