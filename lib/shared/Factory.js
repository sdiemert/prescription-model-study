...
function createEvent(e) {
    ...
    if (e.type === 'medication' || e.type === 'AdministrationEvent') {
        return new AdministrationEvent(e.datetime, e.dose, e.units, e.substance);
    } else if (e.type === 'food' || e.type === 'FoodEvent') {
        return new FoodEvent(e.datetime, e.substance);
    } else if (e.type === 'awake' || e.type === 'AwakeEvent') {
        return new AwakeEvent(e.datetime);
    } else if (e.type === 'sleep' || e.type === 'SleepEvent') {
    ...
    if (unit === "day") {
        ...
        while (i !== -1 && i < data.length) {
            ...
            if (i !== -1) {
                ...
                for (var k = (!firstPass ? j+1 : 0); k <= i; k++) {
                    tmp.addEvent(E[k]);
                }
