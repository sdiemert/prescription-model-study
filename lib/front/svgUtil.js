 ...
 * @param line {string}
 */
function makeCircle(id, x, y, r, fill, line) {
    var e = document.createElementNS(SVGNameSpace, "circle");
    e.setAttribute("cx", x);
    e.setAttribute("cy", y);
    e.setAttribute("r", r);
    e.setAttribute("id", id);
    e.setAttribute("style", "fill:"+ fill +"; stroke:"+line+";");
...
function makeImage(id, path, x, y, w, h){
    ...
    e.setAttribute("x", x+"px");
    e.setAttribute("y", y+"px");
 ...
 * @param c {string}
 */
function makeImageCircle(id, path, x, y, r, c){

   var g = $(makeGroup(id));
   g.append(makeCircle(null, x, y, r, c));
   g.append(makeImage(null, path, x-r/2, y-r/2, r, r));
   return g;
...
module.exports = {
    ...
    makeSVG : makeSVG,
    makeGroup : makeGroup,
    makeImageCircle : makeImageCircle
};
